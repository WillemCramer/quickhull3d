import { getPlaneNormal, pointLineDistance } from "./Math";
...
export default class QuickHull {
  ...
  computeExtremes() {
    ...
    for (i = 0; i < 3; i += 1) {
      min[map[i]] = max[map[i]] = this.vertices[0].point[map[i]];
    }
    ...
    for (i = 1; i < this.vertices.length; i += 1) {
      ...
      for (j = 0; j < 3; j += 1) {
        if (point[map[j]] < min[map[j]]) {
          min[map[j]] = point[map[j]];
          minVertices[j] = vertex;
        }
      }
      // update the max coordinates
      for (j = 0; j < 3; j += 1) {
        if (point[map[j]] > max[map[j]]) {
          max[map[j]] = point[map[j]];
  ...
  createInitialSimplex() {
    ...
    let indexMax = 0;
    for (i = 0; i < 3; i += 1) {
      const distance = max[i].point[map[i]] - min[i].point[map[i]];
    ...
    this.vertices.forEach((vertex) => {
      if (!vertex.point.subtract) {
        throw new Error("vertex.point is not a Vector3");
      }
    });
    ...
    const distPO = Vector3.Dot(v0.point, normal);
    maxDistance = -1;
    for (i = 0; i < this.vertices.length; i += 1) {
      const vertex = this.vertices[i];
      if (vertex !== v0 && vertex !== v1 && vertex !== v2) {
        const distance = Math.abs(Vector3.Dot(normal, vertex.point) - distPO);
        if (distance > maxDistance) {
          maxDistance = distance;
          v3 = vertex;
        }
  ...
  collectFaces(skipTriangulation) {
    ...
    for (let i = 0; i < this.faces.length; i += 1) {
      ...
      if (skipTriangulation) {
        faceIndices.push(Vector4.FromArray(indices));
      } else {
        for (let j = 0; j < indices.length - 2; j += 1) {
          faceIndices.push(
            new Vector3(indices[0], indices[j + 1], indices[j + 2])
          );
        }
      }
    }
  ...
  computeHorizon(eyePoint, crossEdge, face, horizon) {
    ...
    //         | <== not visible face
    //
    //  Vector3.Dot(v, visible face normal) - visible face offset > this.tolerance
    //
    do {
   ...
   *
   *    Vector3.Dot(centroid smaller face, larger face normal) - larger face offset > -tolerance
   *
   ...
   * face i.e.
   *
   *    Vector3.Dot(centroid larger face, smaller face normal) - smaller face offset > -tolerance
   *
   * If true then it means that two faces are non convex (concave), even if the
   * Vector3.Dot(...) - offset value is > 0 (that's the point of doing the merge in the
   * first place)
   *
  ...
  doAdjacentMerge(face, mergeType) {
    ...
    do {
      ...
      //
      //      Vector3.Dot(v, not visible face normal) - not visible offset < tolerance
      //
